...
function createElement(tag, attrs, ...children) {
  ...
  for (const [key, value] of Object.entries(attrs)) {
    if (key.startsWith("on")) {
      ...
      if (!eventHandlersMap.has(vnode.id)) {
        eventHandlersMap.set(vnode.id, {});
      }
      eventHandlersMap.get(vnode.id)[event] = value;
    }
  }
...
const eventsToDelegate = [
  ...
  'input', 'contextmenu', 'wheel', 'copy', 'cut', 'paste',
...
rootElement.addEventListener('click', handleEvent, true); 
