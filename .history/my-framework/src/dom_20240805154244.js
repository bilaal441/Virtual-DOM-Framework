  ...
  render() {
    ...
    for (const [key, value] of Object.entries(this.attrs)) {
      if (key.startsWith("on")) {
        console.log(key, this.attrs["data-id"]);
        const event = key.slice(2).toLowerCase();

        if (!eventHandlersMap.has(this.attrs["data-id"])) {
          eventHandlersMap.set(this.attrs["data-id"], {});
        }

        eventHandlersMap.get(this.attrs["data-id"])[event] = value;
  ...
  switch (patches.type) {
    ...
    case "REMOVE": {
      ...
      if (el && parent && parent.contains(el)) {
        eventHandlersMap.delete(patches.id); // Clean up event handlers
      ...
      attrPatches.forEach(({key, value}) => {
        if (value !== undefined) {
          if (key === "value") {
            ...
            console.log(key, value, attrPatches["data-id"]);
            const event = key.slice(2).toLowerCase();

            if (!eventHandlersMap.has(attrPatches["data-id"])) {
              eventHandlersMap.set(attrPatches["data-id"], {});
            }

            eventHandlersMap.get(attrPatches["data-id"])[event] = value;
          } else {
            element.setAttribute(key, value);
          }
        } else {
